...
/*   Updated: 2018/10/02 10:33:34 by asenat           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
						...
						t_segment **segments)
{
	...
	if (!(msection = (const t_msection*)safe_access(cmd, 
					get_struct_size(SEGMENT_CMD, map->type), map->size)))
		return (0);
	i = 0;
	while (i < nsects)
	{
		...
		if (!(msection = (const t_msection*)safe_access(msection,
					get_struct_size(SECTION, map->type), map->size)))
				return (0);
	}
	return (1);
}
